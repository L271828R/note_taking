...
function M.goto_note()
  ...
  -- First, check if cursor is on a link
  local cursor_line = vim.api.nvim_get_current_line()
  local cursor_col = vim.api.nvim_win_get_cursor(0)[2]

  -- Find if we're inside a [[id|title]] or [[id]] link
  local link_under_cursor = nil
  for start_pos, id_match, end_pos in cursor_line:gmatch("()%[%[([0-9a-f]+)[^%]]*%]%]()") do
    -- Check if cursor is within this link (accounting for 0-based indexing)
    if cursor_col >= start_pos - 1 and cursor_col < end_pos - 1 then
      link_under_cursor = id_match
      write_log("Found link under cursor: " .. id_match)
      break
    end
  end

  -- If cursor is on a link, jump directly to it
  if link_under_cursor then
    local map = load_map()
    if map[link_under_cursor] then
      local target_path = vault_dir .. "/" .. map[link_under_cursor].path
      write_log("Jumping directly to: " .. target_path)
      vim.cmd("edit " .. vim.fn.fnameescape(target_path))
      return
    else
      vim.notify("Link target not found in map: " .. link_under_cursor, vim.log.levels.WARN)
      return
    end
  end

  -- Otherwise, show all links in the file
...
-- Position cursor at top of file when opening Markdown files (skip testbank.md)
vim.api.nvim_create_autocmd({"BufEnter", "BufWinEnter", "BufReadPost"}, {
  pattern = "*.md",
  callback = function()
    local name = vim.fn.expand("%:t")
    if name == "testbank.md" then return end
...
-- 13. find_note: fuzzy search and open any note
function M.find_note()
  write_log("find_note: start")
  fzf.files({
    cwd        = notes_dir,
    prompt     = "Find note ▶︎ ",
    file_icons = false,
...
-- 15. goto_next_question: jump to the first empty # Question slot in testbank.md
function M.goto_next_question()
  local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)

  -- Find the first # Question whose next non-blank line is # Answer or EOF
  for i, line in ipairs(lines) do
    if line:match("^#%s+Question%s*$") then
      -- Check if this question has no content (next non-blank is # Answer or end)
      local empty = true
      for j = i + 1, #lines do
        local next = lines[j]
        if next:match("^%s*$") then
          -- blank line, keep scanning
        elseif next:match("^#%s+Answer") then
          break -- empty question found
        else
          empty = false
          break
        end
  ...
  -- No empty slot — append a new Question/Answer block and jump there
  local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false)
  local new_lines = { "", "# Question", "", "", "# Answer", "" }
  vim.api.nvim_buf_set_lines(0, #lines, #lines, false, new_lines)
  -- Jump to the new # Question line
  vim.api.nvim_win_set_cursor(0, { #lines + 2, 0 })
end

-- Autocmd: in testbank.md, jump to end on open + bind <leader>nq
vim.api.nvim_create_autocmd({"BufReadPost"}, {
  pattern = "*/testbank.md",
  callback = function()
    M.goto_next_question()
    vim.keymap.set("n", "<leader>nq", M.goto_next_question,
      { buffer = true, desc = "Jump to next Question slot in testbank" })
  end,
  group = markdown_group,
})
...
vim.keymap.set("n", "<leader>s",  M.find_note,   { desc = "Search and open note" })
